...
public:	
	void obj2BVH(vector<triangle> &trangleList,glm::vec3 color=glm::vec3(0.5f,0.5f,0.5f),bool islight=false){
		unsigned int index = 0;//indices的顺序
		for (unsigned int i = 0; i < meshes.size(); i++){
			Mesh tmp = meshes[i];
			while (index < tmp.indices.size()){
				trangleList.push_back(triangle(tmp.vertices[tmp.indices[index++]], tmp.vertices[tmp.indices[index++]], tmp.vertices[tmp.indices[index++]],color, islight));
			}
		}	
	}
};
